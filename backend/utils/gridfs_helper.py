...
async def get_pdf_from_gridfs(file_id: str) -> bytes:
    ...
    try:
        ...
        grid_out = await database.fs.open_download_stream(ObjectId(file_id))
...
async def get_pdf_metadata(file_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        file_doc = await database.db.fs.files.find_one({"_id": ObjectId(file_id)})
        return file_doc
...
async def delete_pdf_from_gridfs(file_id: str) -> bool:
    ...
    try:
        ...
        await database.fs.delete(ObjectId(file_id))
...
async def file_exists_in_gridfs(file_id: str) -> bool:
    ...
    try:
        ...
        count = await database.db.fs.files.count_documents({"_id": ObjectId(file_id)})
        return count > 0
